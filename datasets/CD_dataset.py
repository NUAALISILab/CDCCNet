...
class ImageDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.is_train:
            [img, img_B], _, [img_color, img_B_color] = self.augm.transform([img, img_B],[], to_tensor=self.to_tensor)
            return {'A': img, 'B': img_B, 'name': name, 'A_color': img_color, 'B_color': img_B_color}
        else:
...
class CDDataset(ImageDataset):
    ...
    def __init__(self, root_dir, img_size, split='train', is_train=True, label_transform=None,image_type='png',
                to_tensor=True):
    ...
    def __getitem__(self, index):
        ...
        if self.label_transform == 'norm':
            label = label // 255
        outs = self.augm.transform([img_A, img_B], [label], to_tensor=self.to_tensor)

        if self.is_train:
            [img_A, img_B], [label], [img_color, img_B_color] = outs
            return {'name': name, 'A': img_A, 'B': img_B, 'L': label, 'A_color': img_color, 'B_color': img_B_color}
        else:
            [img_A, img_B], [label] = outs
