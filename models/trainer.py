...
class CDTrainer():
    ...
    def __init__(self, args, dataloaders):
        ...
        if args.optimizer == "sgd":
            self.optimizer_G = optim.SGD(self.net_G.parameters(), lr=self.lr,
                                     momentum=0.9,
                                     weight_decay=5e-4)
            max_iter = args.max_epochs * len(dataloaders['train'])
            power = 0.9

            def poly_lr_lambda(step):
                return (1 - min(step, max_iter) / max_iter) ** power

            self.exp_lr_scheduler_G = LambdaLR(self.optimizer_G, lr_lambda=poly_lr_lambda)
        ...
        if args.vis_act:
            self.act_vis_dir = os.path.join(self.vis_dir, 'activations')
            os.makedirs(self.act_vis_dir, exist_ok=True)
    ...
    def _load_pretarined_checkpoint(self, ckpt_file=None):
        if ckpt_file:
            ...
            if isinstance(checkpoint, dict) and 'state_dict_ema' in checkpoint:
                state_dict_key = 'state_dict_ema'

            if isinstance(checkpoint, dict) and state_dict_key in checkpoint:
                raw_dict = checkpoint[state_dict_key]
            ...
            enc_dict = self.net_G.encoder_decoder.state_dict()
            filtered = {}
            for k, v in state_dict.items():
                if k in enc_dict:
                    filtered[k] = v
                else:
                    print('delete pretrained weight:', k)
            prefixed = {f'encoder_decoder.{k}': v for k, v in filtered.items()}
            full_dict = self.net_G.state_dict()
            full_dict.update(prefixed)

            # Load the updated dict into the model
            print(f"\n ---start load pretrained model ({len(prefixed)} weights)--- \n")
            self.net_G.load_state_dict(full_dict, strict=False)
            self.net_G.to(self.device)
            self.logger.write('\n')
        else:
    ...
    def _forward_pass_dp(self, batch):
        ...
        if self.is_training :
            ...
            img_in1_color = batch['A_color'].to(self.device)
            img_in2_color = batch['B_color'].to(self.device)
            gt = self.batch['L'].to(self.device).long()
        else:
            img_in1 = batch['A'].to(self.device)
            img_in2 = batch['B'].to(self.device)
        if self.is_training :
            self.G_pred, self.G_pred_c, self.aux_out, self.ACML1, self.ACML2, self.CCDL1, self.CCDL2, self.STCCL, self.PCRCL = self.net_G(img_in1, img_in2, img_in1_color, img_in2_color, gt)
    ...
    def _backward_G_dp(self):
        ...
        aux_gt = gt
        aux_gt = aux_gt.float()
        aux_gt = nn.functional.interpolate(aux_gt, size=self.aux_out.shape[2:], mode='nearest')
        aux_gt = aux_gt.squeeze(1).long()
        self.aux_loss = self._pxl_loss(self.aux_out, aux_gt)

        self.G_loss = ((self._pxl_loss(self.G_pred, gt) + self._pxl_loss(self.G_pred_c, gt)) / 2 + 0.4 * self.aux_loss + 0.1 * (self.ACML1 + self.ACML2) + 0.1 * (self.CCDL1 + self.CCDL2)+ 0.3 * self.STCCL + 0.3 * self.PCRCL)
    ...
    def train_models_dp(self, pretrain=None):
        if pretrain:
            ...
            print('>>> loaded pretrained backbone:', pretrain)
