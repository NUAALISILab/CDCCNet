...
def cv_rotate(image, angle, borderValue):
    """
    rot angle,  fill with borderValue   旋转角度，并用 borderValue 填充边界
    """
    # grab the dimensions of the image and then determine the center    获取图像的尺寸，然后确定其中心点
    (h, w) = image.shape[:2]    # 将图像的高度和宽度分别赋值给变量 h 和 w
    (cX, cY) = (w // 2, h // 2)

    # grab the rotation matrix (applying the negative of the angle to rotate clockwise), then grab the sine and cosine  获取旋转矩阵（通过负角度实现顺时针旋转），然后提取正弦和余弦值
    # (i.e., the rotation components of the matrix) （即旋转矩阵的旋转分量）
    # -angle位置参数为角度参数负值表示顺时针旋转; 1.0位置参数scale是调整尺寸比例（图像缩放参数），建议0.75
    M = cv2.getRotationMatrix2D((cX, cY), -angle, 1.0)
    cos = np.abs(M[0, 0])
    sin = np.abs(M[0, 1])

    # compute the new bounding dimensions of the image  计算图像旋转后的新边界尺寸
    nW = int((h * sin) + (w * cos))
    nH = int((h * cos) + (w * sin))
    ...
    if isinstance(borderValue, int):
        ...
        values = borderValue
    # perform the actual rotation and return the image  执行实际的旋转操作并返回旋转后的图像
    return cv2.warpAffine(image, M, (nW, nH), borderValue=values)


def pil_resize(img, size, order):   # 调整图像大小以匹配模型输入尺寸
...
def pil_rescale(img, scale, order): # 根据缩放比例调整图像的大小
...
def pil_rotate(img, degree, default_value):
    ...
    img = Image.fromarray(img)  # 将输入的 NumPy 数组转换为 PIL 图像对象
    if img.mode =='RGB':
        # set img padding == default_value
        img2 = img.convert('RGBA')
        rot = img2.rotate(degree, expand=1) # 对图像进行旋转，expand=1 表示旋转后的图像尺寸会扩大以容纳整个图像
        fff = Image.new('RGBA', rot.size, values)  # 灰色 创建一个与旋转后图像大小相同的 RGBA 图像，并用 values 填充
        out = Image.composite(rot, fff, rot)    # 使用旋转图像的 alpha 通道作为掩码，将旋转图像与填充图像合成
        img = out.convert(img.mode) #  将合成后的图像转换回原始模式（RGB）
...
def random_resize_long_image_list(img_list, min_long, max_long):    # 对图像列表中的每张图像进行随机长边缩放
...
def random_scale_list(img_list, scale_range, order):    #   对图像列表中的每张图像进行随机缩放
    """
        输入：图像列表
    """
...
def random_rotate_list(img_list, max_degree, default_values):    #   对图像列表中的每张图像进行随机旋转
...
def random_lr_flip_list(img_list):  # 对图像列表中的每张图像进行随机水平翻转（左右翻转）
...
def get_random_crop_box(imgsize, cropsize): # 计算随机裁剪图像的裁剪框坐标
    h, w = imgsize
...
def random_crop_list(images_list, cropsize, default_values):    # 对图像列表中的每张图像进行随机裁剪
...
def top_left_crop(img, cropsize, default_value):    # 从图像的左上角裁剪指定大小的区域，并用指定的值填充裁剪后的图像边界
...
def center_crop(img, cropsize, default_value=0):    # 从图像的中心裁剪指定大小的区域，并用指定的值填充裁剪后的图像边界
